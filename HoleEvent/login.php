...
$numero_do_hole = 0;
$data_fim_evento = '16/05/2024 10:20:20'; 
    ...
    if ($result = $db->db->execPreparedStmt($query, $params->get())) {
        ...
        if (!empty($row['UID'])) {
            ...
            if ($result = $db->db->execPreparedStmt($query, $params->get())) {
                ...
                if (!empty($row['Holes'])) {
                  $numero_do_hole = $row['Holes'];
                } else {
                    echo "<html><head><title>Recharge Points - Erro</title></head><body><h2>Erro dados incorretos</h2></body></html>";
